...
//
// 参数：
// 1. int nStartRow - 指定的矩阵行开始
// 2. int nStartCol - 指定的矩阵列开始
// 3. int nRows - 指定的矩阵行数
// 4. int nCols - 指定的矩阵列数
// 5. 值矩阵 value - 指定元素的值
//
// 返回值：BOOL 型，说明设置是否成功
//////////////////////////////////////////////////////////////////////
BOOL CMatrix::SetElements(int nStartRow, int nStartCol, int nRow, int nCol, CMatrix value)
{
	if (nStartCol < 0 || nStartCol >= m_nNumColumns || nStartRow < 0 || nStartRow >= m_nNumRows)
		return FALSE;						// array bounds error
	if (nCol < 0 || nCol >= m_nNumColumns || nRow < 0 || nRow >= m_nNumRows)
		return FALSE;						// array bounds error
	if (m_pData == NULL)
	...
	for(int i = 0; i < nRow; i ++)
		...
		{
			m_pData[nStartCol + nCol + (nStartRow + i) * m_nNumColumns] =
...
//
// 参数：
// 1. double value - 与指定矩阵相除的实数
//
// 返回值：CMatrix型，指定矩阵与value相除之商
//////////////////////////////////////////////////////////////////////
CMatrix	CMatrix::operator/(double value) const
{
	// 构造目标矩阵
...
// 矢量点积
//
double CMatrix::Dot(const CMatrix &other)
{
	// 检查行数是否符合要求
	ASSERT (m_nNumRows == other.GetNumRows());
	// 检查列数是否符合要求
	ASSERT (m_nNumColumns == other.GetNumColumns());
